...
const common = {
  ...
  preserve: ["travelpack", "attachments", "access-links", "site-url", "deployment-id", "host-secrets"],
};
...
if (!supportedProducts.has(current.product) || !current.skillVersion || !current.dataSchemaVersion) {
  ...
  result = migration ? {
    ...
    migration,
    steps: ["backup", "copy-on-write-migration", "validate-migrated-travelpack", "deploy-to-existing-project", "publish", "verify", "write-target-manifest"],
  } : {
    ...common,
